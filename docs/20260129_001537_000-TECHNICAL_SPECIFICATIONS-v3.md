...
- **Process**:
  ...
  2.  **Resource Copy**: Copies selected local images to the project directory.
  3.  **Edge-TTS**: Generates audio from the _provided script_.
  4.  **Assembly**: Uses the copied images and generated audio to build the video.
- **Logic**:
  - If `manualScript` and `manualImages` are present in the IPC payload, the backend automatically switches to this flow.
  - Placeholder generation is skipped.
